...
const FilterSections = ({ myRef, hiddenGatogry, categoryId, isFullPage, onClose }) => {
  ...
  const { SubGatogryOptions } = useGetSubGatogry(activeCategoryId);
  
  // Identify Active Top-Level Category Type
  const selectedCategoryObj = GatogryOptions?.find(c => c.value?.toString() === activeCategoryId?.toString());
  const selectedCategoryName = selectedCategoryObj ? selectedCategoryObj.name.toLowerCase() : "";
  const isCars = selectedCategoryName.includes("car") || selectedCategoryName.includes("سيار") || selectedCategoryName.includes("motor");
  ...
  return (
    <motion.div initial={isFullPage ? { opacity: 0, y: 20 } : false} animate={isFullPage ? { opacity: 1, y: 0 } : false} exit={isFullPage ? { opacity: 0, y: 20 } : false} transition={{ duration: 0.3 }} className={overlayClasses}>
      <div className="relative flex items-center justify-between p-2 mb-2">
        ...
        {isFullPage && (
          <div className="flex items-center gap-3">
            ...
            <button onClick={onClose} className="p-2 text-gray-500 hover:text-primary transition rounded-full hover:bg-gray-100"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col">
        ...
        {isCars && (
          <FilterBlock title={selectedContent[localizationKeys.brand]} defaultExpanded={true}>
             <MultiButtonFilter values={categoryId ? brandOptions : allBrands} name="brands" isMultiSelect={true} myRef={myRef} />
          </FilterBlock>
        )}

        <FilterBlock title={selectedContent[localizationKeys.condition]} defaultExpanded={false}>
          <MultiButtonFilter name="usageStatus" values={[{ name: selectedContent[localizationKeys.new], value: "NEW" }, { name: selectedContent[localizationKeys.used], value: "USED" }]} isMultiSelect={false} myRef={myRef} variant="button" />
        </FilterBlock>

        <FilterBlock title={selectedContent[localizationKeys.price]} isPadding={isFullPage ? false : true} defaultExpanded={true}>
          <div className={isFullPage ? "px-4 pb-4 w-full max-w-3xl mx-auto" : ""}>
