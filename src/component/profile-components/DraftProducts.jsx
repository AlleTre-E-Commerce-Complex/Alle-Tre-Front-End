...
const DraftProducts = ({ OnReload }) => {
  ...
  useEffect(() => {
    if (search.includes("page") && search.includes("perPage"))
      run(
        authAxios
          .get(`${api.app.auctions.getAlldraft}${search}&status=DRAFTED&type=LISTED_PRODUCT`)
          .then((res) => {
            setDraftProductData(res?.data?.data);
      ...
      {draftProductData?.length === 0 ? (
        <div className="flex flex-col items-center justify-center min-h-[300px] bg-white dark:bg-[#1A1F2C] border border-gray-200 dark:border-[#2C3241] rounded-2xl p-8 mt-6 shadow-sm mx-auto w-full">
          <div className="bg-primary/10 dark:bg-primary/20 p-5 rounded-full mb-5">
            <IoDocumentTextOutline className="w-12 h-12 text-primary" />
          </div>
          <p className="text-gray-500 dark:text-gray-400 text-center max-w-sm text-md font-medium">
            {
              selectedContent[
        ...
        <div>
          {draftProductData?.map((e) => (
            <ProductRowTable
              ...
              price={e?.ProductListingPrice || e?.product?.ProductListingPrice}
              goToDetails={routes.app.listProduct.default} // For drafts, we usually go back to the listing page
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center mt-8 pt-6 border-t border-gray-200 dark:border-gray-800">
            <p className="text-gray-500 dark:text-gray-400 text-sm mb-4 sm:mb-0">
              {selectedContent[localizationKeys.showing]} {draftProductData?.length || 0} {totalItems ? `${selectedContent[localizationKeys.of]} ${totalItems}` : ""} {selectedContent[localizationKeys.products]}
            </p>
