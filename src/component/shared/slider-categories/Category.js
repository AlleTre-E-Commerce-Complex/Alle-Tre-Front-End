...
const Category = ({ img, title, id, view }) => {
  ...
  <div className="absolute inset-0 flex items-center justify-center">
    <div className="bg-primary bg-opacity-95 text-white text-xs md:text-sm font-bold px-4 md:px-2 py-2 rounded-xl shadow-lg ">
      {selectedContent[localizationKeys.comingSoon].toUpperCase()}
        ...
        <div
          ...
          className={`w-26 h-24 md:w-36 md:h-36 bg-white ${
            !isDisabled
              ? "opacity-40 cursor-not-allowed"
              : "hover:bg-primary/10 cursor-pointer"
          } transition-all duration-300 ease-in-out rounded-full flex items-center justify-center p-3`} 
        >
          <div
            className={`w-20 h-20 md:w-28 md:h-28 rounded-full ${
              !isDisabled ? "bg-gray-med" : "bg-primary group-hover:bg-primary-dark"
      ...
      <p
        className={`mt-3 text-gray-700 font-medium text-sm md:text-base text-center ${
